...
class Decrypt extends Component {
  ...
  componentWillMount() {
    const { title, fetchKeysForVaultEntryIfNeeded, vault } = this.props;
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.title !== nextProps.title) {
      const keyKeys = nextProps.vault
        .getIn(["entries", nextProps.title])
        .map(e => e.get("key"));
  ...
  getEncryptingKeys = () => {
    ...
    const keyIDs = vault
      ...
      .map(e => e.get("key"))
      .toSet();
    const encryptingKeys = vault
      .getIn(["titleToKeys", title], List())
      .filter(k => keyIDs.contains(k.get("id")));
  ...
  render() {
    ...
    return (
      ...
      <div
        className={classNames("vaultEntryDecrypt", { greyContainer: !version })}
      >
        {state === NONE ? (
          <div>
            {encryptingKeys.map(key => (
              <button
                key={key.get("id")}
                className="purple"
                onClick={this.promptForSecret(key.get("id"))}
              >
                {key.get("name")}
              </button>
            ))}
          ...
          <div className="decryptedContainer">
            <div className="decrypted">
              ...
              />
              <button
                className="nobackground copyButton"
                onClick={this.copy(this.id)}
              >
                {copySuccess ? (
          ...
          <div className="ciphertextContainer">
            ...
            <div className="ciphertext">
              ...
              <button
                className="nobackground copyButton"
                onClick={this.copy("ciphertextText")}
              >
                {copySuccess ? (
                  <i className="fa fa-check-circle" />
                ) : copyFailed ? (
...
export default connect(
  ...
  dispatch => ({
    ...
    fetchKeysForVaultEntryIfNeeded: (title, keyKeys) =>
      dispatch(fetchKeysForVaultEntryIfNeeded(title, keyKeys)),
  })
)(Decrypt);
