...
class PreviousVersions extends Component {
    ...
    render() {
        ...
        const versions = vault.getIn(['entries', title], List()).map(e => e.get('version'))
            .toSet()
            .sort((v1, v2) => v1 < v2)
            .skip(1);
            ...
            <div className="vaultEntryPreviousVersions greyContainer">
                ...
                <div key={ v }>
                    <div onClick={ this.toggle(v) } className="version">
                        { open.contains(v) ? <i className="fa fa-caret-down triangle"></i> : <i className="fa fa-caret-right triangle"></i> }
                        Version { v }
                    </div>
                    {
                    open.contains(v)
